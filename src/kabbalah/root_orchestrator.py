...
class RootOrchestrator:
    ...
    def decompose_specification(self, specification, run_id: str) -> List[DomainBranch]:
        """
        ...
        try:
            ...
            for domain in specification.domains:
                ...
                branch = DomainBranch(
                    ...
                    metadata={"created_at": datetime.utcnow().timestamp(), "domain": domain},
                )
            ...
            if isinstance(e, DecompositionError):
                raise
            raise DecompositionError(f"Failed to decompose specification: {str(e)}") from e

    def execute_branches(self, branches: List[DomainBranch]) -> Dict[str, BranchResult]:
        """
        ...
        try:
            ...
            while len(executed) < len(branches):
                ...
                ready_branches = [
                    b
                    for b in branches
                    if b.branch_id not in executed
                    and all(
                        domain_to_branch.get(dep) in executed or dep not in domain_to_branch for dep in b.dependencies
                    )
                ]
            ...
            if isinstance(e, ExecutionError):
                raise
            raise ExecutionError(f"Failed to execute branches: {str(e)}") from e
    ...
    def _execute_branch(self, branch: DomainBranch) -> BranchResult:
        ...
        try:
            ...
            leaf_nodes = domain_orch.spawn_leaf_nodes(branch=branch, run_id=branch.run_id, branch_id=branch.branch_id)
