...
class SpecificationParser:
    ...
    def _parse_content(
        ...
        raise ParsingError("Could not parse content as JSON or YAML. " "Please provide valid JSON or YAML format.")
    ...
    def _parse_yaml(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ParsingError("YAML support requires PyYAML to be installed") from exc
        except Exception as e:
            raise ParsingError(f"Invalid YAML: {str(e)}") from e
    ...
    def _validate_specification(self, data: Dict[str, Any]) -> None:
        ...
        if not isinstance(run_id, str) or not run_id.startswith("run_"):
            raise ValidationError(f"Invalid run_id format: {run_id}. " "Expected format: run_YYYY_MM_DD_NNN")
