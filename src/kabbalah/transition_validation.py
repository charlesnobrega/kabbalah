...
class TransitionValidationModule:
    ...
    def register_agent(self, agent_id: str, agent_role: str, metadata: Optional[Dict] = None) -> None:
        """
    ...
    def update_agent_health(
        self, agent_id: str, status: AgentStatus, is_responsive: bool, error_count: int = 0, warning_count: int = 0
    ) -> None:
        """
    ...
    def validate_agent_health(self) -> Tuple[bool, List[AgentHealthStatus]]:
        ...
        for _agent_id, agent in self._agent_registry.items():
            # Agent is healthy if:
            # 1. Status is HEALTHY
            # 2. Agent is responsive
            # 3. No critical errors (error_count == 0)
            is_healthy = agent.status == AgentStatus.HEALTHY and agent.is_responsive and agent.error_count == 0
    ...
    def validate_transition(self, from_mode: str, to_mode: str) -> TransitionValidationResult:
        """
        ...
        """
        result = TransitionValidationResult(is_valid=True, timestamp=time.time(), from_mode=from_mode, to_mode=to_mode)
        ...
        if from_mode == "DAY1" and to_mode == "DAY2":
            ...
            if not result.is_valid:
                ...
                result.error_message = (
                    f"Transition from {from_mode} to {to_mode} failed. " f"Failed checks: {', '.join(failed_checks)}"
                )
