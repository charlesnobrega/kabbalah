...
agent_statuses = st.sampled_from(
    [AgentStatus.HEALTHY, AgentStatus.UNHEALTHY, AgentStatus.UNKNOWN, AgentStatus.INITIALIZING, AgentStatus.DEGRADED]
)
...
class TestAgentRegistrationProperties:
    ...
    @given(agent_id1=agent_ids, agent_id2=agent_ids, agent_role=agent_roles)
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
...
class TestAgentHealthUpdateProperties:
    ...
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_health_update_changes_agent_status(self, agent_id, agent_role, status, is_responsive, error_count):
    ...
    @given(agent_id=agent_ids, agent_role=agent_roles, error_count=error_counts, warning_count=warning_counts)
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_health_update_preserves_error_and_warning_counts(self, agent_id, agent_role, error_count, warning_count):
        """Property: Error and warning counts are preserved in updates."""
        module = TransitionValidationModule()
        module.register_agent(agent_id, agent_role)

        module.update_agent_health(agent_id, AgentStatus.HEALTHY, True, error_count, warning_count)
...
class TestAgentHealthValidationProperties:
    ...
    @given(agent_id=agent_ids, agent_role=agent_roles, status=agent_statuses)
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    ...
    @given(agent_id=agent_ids, agent_role=agent_roles, error_count=st.integers(min_value=1, max_value=100))
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
...
class TestTransitionValidationProperties:
    ...
    @given(
        ...
        status=st.sampled_from(
            [AgentStatus.UNHEALTHY, AgentStatus.UNKNOWN, AgentStatus.INITIALIZING, AgentStatus.DEGRADED]
        ),
    )
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    ...
    @given(
        from_mode=st.sampled_from(["BOOTSTRAP", "DAY1", "DAY2"]), to_mode=st.sampled_from(["BOOTSTRAP", "DAY1", "DAY2"])
    )
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
...
class TestValidationHistoryProperties:
    ...
    @given(num_validations=st.integers(min_value=1, max_value=10), agent_id=agent_ids, agent_role=agent_roles)
    @settings(max_examples=50, suppress_health_check=[HealthCheck.too_slow])
    def test_all_validations_recorded_in_history(self, num_validations, agent_id, agent_role):
    ...
    @given(num_validations=st.integers(min_value=1, max_value=10), agent_id=agent_ids, agent_role=agent_roles)
    @settings(max_examples=50, suppress_health_check=[HealthCheck.too_slow])
    def test_validation_history_is_ordered(self, num_validations, agent_id, agent_role):
...
class TestResetStateProperties:
    ...
    @given(num_agents=st.integers(min_value=1, max_value=10), agent_role=agent_roles)
    @settings(max_examples=50, suppress_health_check=[HealthCheck.too_slow])
    ...
    @given(num_validations=st.integers(min_value=1, max_value=10), agent_id=agent_ids, agent_role=agent_roles)
    @settings(max_examples=50, suppress_health_check=[HealthCheck.too_slow])
    def test_reset_clears_validation_history(self, num_validations, agent_id, agent_role):
...
class TestDay1ToDay2TransitionValidation:
    ...
    **Validates: Requirements 12.5**
    ...
    @given(num_agents=st.integers(min_value=1, max_value=5), agent_role=agent_roles)
    @settings(max_examples=50, suppress_health_check=[HealthCheck.too_slow])
