...
Tests the GoogleGeminiProvider implementation with real API calls.
"""
...
from src.kabbalah.providers import GoogleGeminiProvider, ProviderResponse
...
class TestGoogleGeminiProvider:
    ...
    def test_execute_request_simple(self, provider):
        ...
        if "429" in str(response.error) or "quota" in str(response.error).lower():
            pytest.skip("Google Gemini API quota exceeded - expected on free tier")
    ...
    def test_calculate_cost(self, provider):
        ...
        cost = provider.calculate_cost(1000, "gemini-2.5-flash")
    ...
    def test_stream_request(self, provider):
        ...
        assert last_response.content is not None
        assert last_response.error is None
    
    def test_multiple_requests(self, provider):
        """Test multiple sequential requests"""
        for i in range(2):  # Reduced from 3 to 2 to avoid rate limiting
            request = {
                "model": "gemini-2.5-flash",
                "messages": [
            ...
            # Allow either successful response or rate limit error
