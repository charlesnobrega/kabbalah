...
operation_names = st.text(alphabet=st.characters(blacklist_categories=("Cc", "Cs")), min_size=1, max_size=50).filter(
    lambda x: x.strip()
)

# Strategy for generating trace IDs (run_id:branch_id:leaf_id)
trace_ids = st.builds(
    lambda run_index, branch_index, leaf_index: f"run_{run_index}:branch_{branch_index}:leaf_{leaf_index}",
    run_index=st.integers(min_value=1, max_value=1000),
    branch_index=st.integers(min_value=1, max_value=100),
    leaf_index=st.integers(min_value=1, max_value=100),
)

# Strategy for generating input dictionaries
input_dicts = st.dictionaries(
    keys=st.text(alphabet=st.characters(blacklist_categories=("Cc", "Cs")), min_size=1, max_size=30).filter(
        lambda x: x.strip()
    ),
    values=st.one_of(
        st.integers(),
    ...
    **Validates: Requirements 2.3.7**
    ...
    def test_precondition_rejects_invalid_inputs(self, op_name, inputs, trace_id):
        ...
        contract = OperationContract(
            operation_name=op_name, preconditions=[precond_positive_value], postconditions=[], output_parser=None
        )
    ...
    def test_multiple_preconditions_all_enforced(self, op_name, inputs, trace_id):
        ...
        contract = OperationContract(
            operation_name=op_name, preconditions=[precond1, precond2], postconditions=[], output_parser=None
        )
...
class TestPostconditionValidationProperty:
    ...
    **Validates: Requirements 2.3.8**
    ...
    def test_postcondition_rejects_invalid_outputs(self, op_name, outputs, trace_id):
        ...
        contract = OperationContract(
            operation_name=op_name, preconditions=[], postconditions=[postcond_positive_result], output_parser=None
        )
...
class TestViolationLoggingProperty:
    ...
    **Validates: Requirements 2.3.9**
    ...
    def test_precondition_violations_logged_with_context(self, op_name, inputs, trace_id):
        ...
        contract = OperationContract(
            operation_name=op_name, preconditions=[precond], postconditions=[], output_parser=None
        )
    ...
    def test_postcondition_violations_logged_with_context(self, op_name, outputs, trace_id):
        ...
        contract = OperationContract(
            operation_name=op_name, preconditions=[], postconditions=[postcond], output_parser=None
        )
    ...
    def test_parser_violations_logged_with_context(self, op_name, inputs, outputs, trace_id):
        ...
        contract = OperationContract(operation_name=op_name, preconditions=[], postconditions=[], output_parser=parser)
    ...
    def test_multiple_violations_all_logged(self, op_names):
        ...
        for i, op_name in enumerate(op_names):
            module.validate_preconditions(op_name, {"index": i}, f"run_001:branch_001:leaf_{i:03d}")
