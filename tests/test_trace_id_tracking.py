...
class TestTraceIDConstruction:
    ...
    def test_construct_trace_id_invalid_run_id(self):
        ...
        with pytest.raises(TraceIDError):
            TraceIDGenerator.construct_trace_id("invalid_run_id", "branch_backend_001", "leaf_backend_001")

    def test_construct_trace_id_invalid_branch_id(self):
        """Test construction rejects invalid branch_id."""
        with pytest.raises(TraceIDError):
            TraceIDGenerator.construct_trace_id("run_2026_04_06_001", "invalid_branch_id", "leaf_backend_001")

    def test_construct_trace_id_invalid_leaf_id(self):
        """Test construction rejects invalid leaf_id."""
        with pytest.raises(TraceIDError):
            TraceIDGenerator.construct_trace_id("run_2026_04_06_001", "branch_backend_001", "invalid_leaf_id")
        ...
        def append_entries(thread_id):
            try:
                for i in range(10):
                    ...
                    log.append_entry(trace_id, f"op_{thread_id}_{i}", "write", "success", time.time(), time.time() + 1)
