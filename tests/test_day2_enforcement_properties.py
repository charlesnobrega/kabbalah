...
operation_names = st.text(alphabet=st.characters(blacklist_categories=("Cc", "Cs")), min_size=1, max_size=100).filter(
    lambda x: x.strip()
)

# Strategy for generating metadata dictionaries
metadata_strategy = st.dictionaries(
    keys=st.text(alphabet=st.characters(blacklist_categories=("Cc", "Cs")), min_size=1, max_size=50).filter(
        lambda x: x.strip()
    ),
    values=st.one_of(
...
class TestDAY2EnforcementProperty:
    ...
    def test_all_bootstrap_operations_blocked_in_day2(self, operation):
        ...
        is_allowed = module.check_operation_allowed(operation, OperationalMode.DAY2)

        # Property: bootstrap operations must be blocked in DAY2
        assert not is_allowed, (
            f"Bootstrap operation {operation.operation_type.value} " f"should be blocked in DAY2 mode"
        )
    ...
    def test_all_bootstrap_operations_violations_logged_in_day2(self, operation):
        ...
        assert error_msg is not None, "Error message should be provided for blocked operation"

        # Property: violation must be logged
        violations = module.violation_log
        assert len(violations) > 0, "At least one violation should be logged for blocked operation"

        # Property: violation must reference the operation
        last_violation = violations[-1]
        assert last_violation.operation == operation, "Violation should reference the blocked operation"

        # Property: violation must be in DAY2 mode
        assert last_violation.current_mode == OperationalMode.DAY2, "Violation should be recorded in DAY2 mode"

        # Property: violation type must be BOOTSTRAP_OPERATION_IN_DAY2
        assert (
            last_violation.violation_type == "BOOTSTRAP_OPERATION_IN_DAY2"
        ), "Violation type should be BOOTSTRAP_OPERATION_IN_DAY2"

    @given(bootstrap_operations, st.lists(bootstrap_operations, min_size=1, max_size=10))
    @settings(
        max_examples=50,
        suppress_health_check=[HealthCheck.too_slow],
    )
    def test_multiple_bootstrap_operations_all_blocked_in_day2(self, first_operation, remaining_operations):
    ...
    def test_bootstrap_operations_allowed_in_bootstrap_mode(self, operation):
        ...
        is_allowed = module.check_operation_allowed(operation, OperationalMode.BOOTSTRAP)

        # Property: bootstrap operations must be allowed in BOOTSTRAP mode
        assert is_allowed, (
            f"Bootstrap operation {operation.operation_type.value} " f"should be allowed in BOOTSTRAP mode"
        )
    ...
    def test_bootstrap_operations_allowed_in_day1_mode(self, operation):
        ...
        is_allowed = module.check_operation_allowed(operation, OperationalMode.DAY1)

        # Property: bootstrap operations must be allowed in DAY1 mode
        assert is_allowed, f"Bootstrap operation {operation.operation_type.value} " f"should be allowed in DAY1 mode"

    @given(bootstrap_operations, st.lists(bootstrap_operations, min_size=1, max_size=5))
    @settings(
        max_examples=50,
        suppress_health_check=[HealthCheck.too_slow],
    )
    def test_violation_log_consistency_across_multiple_operations(self, first_operation, remaining_operations):
        ...
        violations = module.violation_log
        assert len(violations) == len(all_operations), f"Violation log should contain {len(all_operations)} violations"

        # Property: each violation must reference the corresponding operation
        for i, operation in enumerate(all_operations):
            assert violations[i].operation == operation, f"Violation {i} should reference operation {i}"
            assert violations[i].current_mode == OperationalMode.DAY2, f"Violation {i} should be in DAY2 mode"
    ...
    def test_violation_metadata_preserved_in_day2(self, operation):
        ...
        last_violation = violations[-1]
        assert (
            last_violation.operation.metadata == operation.metadata
        ), "Operation metadata should be preserved in violation"

    @given(st.lists(bootstrap_operations, min_size=1, max_size=20))
    @settings(
        max_examples=30,
        suppress_health_check=[HealthCheck.too_slow],
    )
    ...
    def test_day2_blocking_is_deterministic(self, operation):
        ...
        results = [module.check_operation_allowed(operation, OperationalMode.DAY2) for _ in range(5)]

        # Property: all results must be False
        assert all(result is False for result in results), "DAY2 blocking behavior should be deterministic"

        # Property: all results must be identical
        assert len(set(results)) == 1, "All checks should return the same result"
    ...
    def test_violation_timestamp_recorded_in_day2(self, operation):
        ...
        last_violation = violations[-1]
        assert last_violation.timestamp is not None, "Violation timestamp should be recorded"

        # Property: timestamp should be within the recorded time range
        assert (
            before <= last_violation.timestamp <= after
        ), "Violation timestamp should be within the recorded time range"
