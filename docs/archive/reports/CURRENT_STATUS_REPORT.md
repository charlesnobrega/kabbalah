...
All API keys are stored securely in the vault at `D:\Users\charl\.secrets\keys.json`.

Use `secrets_vault.py` to access keys:

```python
from kabbalah.secrets_vault import get_api_key
api_key = get_api_key('openai')
```

**Note**: Keys are never committed to the repository.
